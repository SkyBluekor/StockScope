...
def patch_router(text: str) -> str:
    if "holdings_api_hold1f" in text or "integrations_api_hold1f" in text:
        fail("api/router.py already appears to contain HOLD.1-F explicit routes.")
    ...
    imports = (
        "from app.api import holdings as holdings_api_hold1f\n"
        "from app.api import integrations as integrations_api_hold1f\n\n"
    )
    patched = text.replace(anchor, imports + anchor, 1)

    registrations = """
# HOLD.1-F: register routes explicitly.
# This project already uses explicit add_api_route registration for API
# integration stability; do not depend on nested router copying here.
api_router.add_api_route(
    "/holdings/stocks",
    holdings_api_hold1f.list_stocks,
    methods=["GET"],
    tags=["holdings"],
)
api_router.add_api_route(
    "/holdings/stocks/{stock_id}",
    holdings_api_hold1f.stock_detail,
    methods=["GET"],
    tags=["holdings"],
)
api_router.add_api_route(
    "/holdings/watch",
    holdings_api_hold1f.register_watch,
    methods=["POST"],
    tags=["holdings"],
)
api_router.add_api_route(
    "/holdings/stocks/{stock_id}/watch",
    holdings_api_hold1f.set_watch_state,
    methods=["PATCH"],
    tags=["holdings"],
)
api_router.add_api_route(
    "/holdings/accounts",
    holdings_api_hold1f.list_accounts,
    methods=["GET"],
    tags=["holdings"],
)
api_router.add_api_route(
    "/holdings/manual/buy",
    holdings_api_hold1f.record_manual_buy,
    methods=["POST"],
    tags=["holdings"],
)
api_router.add_api_route(
    "/holdings/manual/{position_id}/sell",
    holdings_api_hold1f.record_manual_sell,
    methods=["POST"],
    tags=["holdings"],
)
api_router.add_api_route(
    "/holdings/manual/{position_id}/correction",
    holdings_api_hold1f.record_manual_correction,
    methods=["POST"],
    tags=["holdings"],
)
api_router.add_api_route(
    "/holdings/stocks/{stock_id}/analysis",
    holdings_api_hold1f.current_analysis,
    methods=["GET"],
    tags=["holdings"],
)
api_router.add_api_route(
    "/holdings/stocks/{stock_id}/analysis/refresh",
    holdings_api_hold1f.refresh_analysis,
    methods=["POST"],
    tags=["holdings"],
)
api_router.add_api_route(
    "/holdings/stocks/{stock_id}/analysis/timeline",
    holdings_api_hold1f.analysis_timeline,
    methods=["GET"],
    tags=["holdings"],
)
api_router.add_api_route(
    "/holdings/stocks/{stock_id}/timeline",
    holdings_api_hold1f.stock_timeline,
    methods=["GET"],
    tags=["holdings"],
)
api_router.add_api_route(
    "/holdings/stocks/{stock_id}/analysis/{market_date}/revisions",
    holdings_api_hold1f.analysis_revisions,
    methods=["GET"],
    tags=["holdings"],
)
api_router.add_api_route(
    "/holdings/kis/sync",
    holdings_api_hold1f.sync_kis_balance,
    methods=["POST"],
    tags=["holdings"],
)
api_router.add_api_route(
    "/integrations/status",
    integrations_api_hold1f.integration_status,
    methods=["GET"],
    tags=["integrations"],
)
api_router.add_api_route(
    "/integrations/{provider}/check",
    integrations_api_hold1f.check_integration,
    methods=["POST"],
    tags=["integrations"],
)
"""
    return patched.rstrip() + "\n" + registrations.lstrip()
...
required = {
    ...
    "/api/holdings/manual/buy",
    "/api/holdings/kis/sync",
    "/api/integrations/status",
    "/api/integrations/{provider}/check",
}
missing = sorted(required - paths)
if missing:
    print("ACTUAL HOLD.1-F-LIKE ROUTES:")
    for path in sorted(
        value for value in paths
        if isinstance(value, str)
        and ("/holdings" in value or "/integrations" in value)
    ):
        print(" -", path)
...
"""
        ...
        print("Modified:")
        print(" - backend/app/api/router.py (explicit add_api_route registration only)")
