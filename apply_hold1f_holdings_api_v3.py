...
def main() -> int:
    ...
    try:
        ...
        print()
        print("=== HOLD.1-F ROUTER REGISTRATION (FastAPI 0.141+ aware) ===")
        route_code = r"""
import os
import sys
import tempfile
...
# FastAPI 0.141+ keeps nested routers as _IncludedRouter objects.
# Their entries do not expose .path directly, so validate the final app contract
# through OpenAPI + real TestClient requests instead of inspecting app.routes.
with tempfile.TemporaryDirectory() as td:
    ...
    if missing:
        print("OPENAPI HOLD.1-F-LIKE ROUTES:")
        for path in sorted(
            value for value in paths
            if "/holdings" in value or "/integrations" in value
        ):
            print(" -", path)
        raise SystemExit(
            "Missing HOLD.1-F OpenAPI routes: " + ", ".join(missing)
        )

    client = TestClient(app)

    stocks = client.get("/api/holdings/stocks")
    assert stocks.status_code == 200, stocks.text
    assert stocks.json() == [], stocks.json()

    accounts = client.get("/api/holdings/accounts")
    assert accounts.status_code == 200, accounts.text
    assert accounts.json() == [], accounts.json()

    status = client.get("/api/integrations/status")
    assert status.status_code == 200, status.text
    assert set(status.json()) == {"kis", "krx", "dart"}
    ...
    assert watched.json()["stock"]["ticker"] == "005930"

    print("HOLDINGS ROUTER: REGISTERED")
    print("INTEGRATIONS ROUTER: REGISTERED")
    print("APP /api PREFIX: VERIFIED")
    print("OPENAPI ROUTES: VERIFIED")
    print("REAL TESTCLIENT ROUTING: VERIFIED")
