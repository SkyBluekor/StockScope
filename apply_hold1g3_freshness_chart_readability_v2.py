...
def main() -> int:
    ...
    try:
        ...
        checks = {
            ...
            "freshness before analysis": (
                "prepare_latest: bool = Query(default=False)" in api_now
                and "await _freshness_service().prepare" in api_now
                and "stored = _history_service(catalog).analyze_latest_confirmed" in api_now
            ),
            "frontend explicit freshness trigger": "prepare_latest=true" in service_now,
            "analysis date alignment guard": "최신 확정 시세 준비일과 실제 분석 기준일이 일치하지 않습니다." in api_now,
        ...
        print("User-click refresh prepares latest confirmed EOD first: PASS")
