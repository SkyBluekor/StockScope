from __future__ import annotations

import argparse
import sys
from datetime import date
from pathlib import Path

BACKEND_ROOT = Path(__file__).resolve().parents[1]
if str(BACKEND_ROOT) not in sys.path:
    sys.path.insert(0, str(BACKEND_ROOT))

from app.backtest.market_store import HistoricalMarketStore
from app.backtest.scanner import StockScannerService
from app.backtest.scanner_quality import (
    AuditHorizons,
    EarlyPruningAuditor,
    PruningVariant,
    add_audit_metadata,
    build_temporal_validation,
    compact_temporal_payload,
    discover_evaluation_dates,
    discover_temporal_evaluation_dates,
    write_outputs,
)


class OfflineAuditKrx:
    """Minimal provider facade. Any attempted market download fails immediately."""

    @staticmethod
    def _today_kst() -> date:
        return date.today()

    async def open_session(self) -> None:
        return None

    async def close_session(self) -> None:
        return None

    @staticmethod
    def request_stats() -> dict[str, int]:
        return {"network_requests": 0}

    @staticmethod
    def budget_snapshot() -> dict[str, int]:
        return {"used": 0, "safe_limit": 0, "remaining": 0}

    async def stock_daily(self, *args, **kwargs):  # pragma: no cover - hard guard
        raise RuntimeError("Scanner pruning audit is offline-only; KRX network access is disabled.")

    async def index_daily(self, *args, **kwargs):  # pragma: no cover - hard guard
        raise RuntimeError("Scanner pruning audit is offline-only; KRX network access is disabled.")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="StockScope Scanner early-pruning quality audit")
    parser.add_argument("--market-store", type=Path, default=None, help="market_history.db path")
    parser.add_argument("--market-scope", default="ALL", choices=["ALL", "KOSPI", "KOSDAQ"])
    parser.add_argument("--dates", default="", help="comma-separated YYYY-MM-DD dates")
    parser.add_argument("--mode", default="standard", choices=["standard", "temporal-validation"])
    parser.add_argument("--sample-size", type=int, default=None)
    parser.add_argument("--min-date-gap", type=int, default=3, help="preferred gap in common trading dates")
    parser.add_argument("--min-required-dates", type=int, default=60)
    parser.add_argument("--market-limit", type=int, default=320, help="expanded market pre-filter limit")
    parser.add_argument("--quick-limit", type=int, default=36, help="expanded quick candidate limit")
    parser.add_argument("--output-dir", type=Path, default=BACKEND_ROOT / "runtime" / "quality_audit" / "pruning")
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    store = HistoricalMarketStore(args.market_store) if args.market_store else HistoricalMarketStore()
    scanner = StockScannerService(OfflineAuditKrx(), market_store=store)
    auditor = EarlyPruningAuditor(scanner, store)

    baseline_market = int(scanner.QUICK_LIMIT_PER_MARKET)
    baseline_quick = int(scanner.DEEP_LIMIT)
    sampling = None

    if args.mode == "temporal-validation":
        sample_size = max(1, int(args.sample_size or 80))
        if args.dates.strip():
            dates = [date.fromisoformat(value.strip()) for value in args.dates.split(",") if value.strip()]
            sampling = {
                "source": "explicit_dates",
                "requested_sample_size": len(dates),
                "selected_sample_size": len(dates),
                "requested_min_date_gap": None,
                "effective_min_date_gap": None,
                "first_selected_date": dates[0].isoformat() if dates else None,
                "last_selected_date": dates[-1].isoformat() if dates else None,
            }
        else:
            dates, sampling = discover_temporal_evaluation_dates(
                store,
                market_scope=args.market_scope,
                sample_size=sample_size,
                min_date_gap=max(1, int(args.min_date_gap)),
                future_horizon_trading_days=20,
                min_required_dates=max(1, int(args.min_required_dates)),
            )
        variants = [
            PruningVariant("BASELINE", baseline_market, baseline_quick),
            PruningVariant("QUICK_EXPANDED", baseline_market, max(baseline_quick, args.quick_limit)),
        ]
    else:
        sample_size = max(1, int(args.sample_size or 20))
        if args.dates.strip():
            dates = [date.fromisoformat(value.strip()) for value in args.dates.split(",") if value.strip()]
        else:
            dates = discover_evaluation_dates(
                store,
                market_scope=args.market_scope,
                sample_size=sample_size,
            )
        variants = [
            PruningVariant("BASELINE", baseline_market, baseline_quick),
            PruningVariant("MARKET_EXPANDED", max(baseline_market, args.market_limit), baseline_quick),
            PruningVariant("QUICK_EXPANDED", baseline_market, max(baseline_quick, args.quick_limit)),
            PruningVariant("BOTH_EXPANDED", max(baseline_market, args.market_limit), max(baseline_quick, args.quick_limit)),
        ]

    if not dates:
        print("No complete local evaluation dates were found. Audit did not call KRX.")
        return 2

    print(
        f"Scanner pruning audit: mode={args.mode}, dates={len(dates)}, "
        f"range={dates[0].isoformat()}..{dates[-1].isoformat()}"
    )
    if sampling:
        print(
            "Temporal sampling: "
            f"requested={sampling.get('requested_sample_size')}, selected={sampling.get('selected_sample_size')}, "
            f"gap={sampling.get('requested_min_date_gap')}->{sampling.get('effective_min_date_gap')}"
        )

    def report_progress(index: int, total: int, as_of: date, run: dict) -> None:
        print(
            f"[{index:>3}/{total}] {as_of.isoformat()} "
            f"{run.get('status')} {run.get('runtime_seconds')}s",
            flush=True,
        )

    payload = auditor.run_dates(
        dates=dates,
        variants=variants,
        market_scope=args.market_scope,
        horizons=AuditHorizons((5, 10, 20)),
        progress_callback=report_progress,
    )
    if args.mode == "temporal-validation":
        payload["mode"] = "temporal-validation"
        payload["sampling"] = sampling or {}
        payload["temporal_validation"] = build_temporal_validation(
            payload,
            baseline_name="BASELINE",
            expanded_name="QUICK_EXPANDED",
            minimum_valid_dates=max(1, int(args.min_required_dates)),
        )
        # Full per-symbol traces made the 20-date c.1 JSON ~28 MB. Temporal mode keeps
        # the evidence needed for date/pair analysis but drops that redundant trace.
        payload = compact_temporal_payload(payload)
    else:
        payload["mode"] = "standard"

    payload = add_audit_metadata(payload, market_store=store, project_root=BACKEND_ROOT.parent)
    paths = write_outputs(payload, output_dir=args.output_dir)
    print(f"Audit complete: {payload['valid_date_count']} valid dates")
    if args.mode == "temporal-validation":
        temporal = payload.get("temporal_validation") or {}
        print(
            f"Top5 changes: {temporal.get('changed_date_count')} / {temporal.get('valid_dates')} "
            f"({temporal.get('change_rate_pct')}%), verdict={temporal.get('verdict')}"
        )
    for kind, path in paths.items():
        print(f"{kind}: {path}")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
