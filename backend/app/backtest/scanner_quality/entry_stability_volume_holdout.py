...
def _ready_rows(rows: list[dict[str, Any]] | None) -> list[dict[str, Any]]:
    return [row for row in (rows or []) if str(row.get("candidate_state") or "") == "READY"]
...
def _select(date_results: list[dict[str, Any]], variant: str, top_n: int) -> list[dict[str, Any]]:
    ...
    for item in date_results:
        ...
        output.extend(_ready_rows(rows)[:top_n])
    return output


def _date_event_r(rows: list[dict[str, Any]]) -> float | None:
    return _mean(row.get("event_r_20d") for row in _ready_rows(rows)[:3])
...
def _date_outcome(current: list[dict[str, Any]], guard: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if cur is None and new is None:
        status = "NO_READY"
    elif cur is None or new is None:
        status = "TIE"
    elif new > cur + 1e-12:
        status = "WIN"
    elif new < cur - 1e-12:
...
def _swaps(current: list[dict[str, Any]], guard: list[dict[str, Any]], analysis_date: str) -> list[dict[str, Any]]:
    c_top, g_top = _ready_rows(current)[:3], _ready_rows(guard)[:3]
...
def _ranking_impact(date_results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    checked = 0
    no_ready_dates = 0
    full_top3_ready_dates = 0
    for item in date_results:
        current = _ready_rows(item.get("current_candidates"))
        guard = _ready_rows(item.get("guard_candidates"))
        if not current and not guard:
            no_ready_dates += 1
            continue
        checked += 1
        full_top3_ready_dates += int(len(current) >= 3 and len(guard) >= 3)
    ...
    return {
        ...
        "full_top3_ready_dates": full_top3_ready_dates,
...
def aggregate_holdout(date_results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for index, block_dates in enumerate(_split_blocks(sorted(by_date)), start=1):
        current_rows = [row for day in block_dates for row in _ready_rows(by_date[day].get("current_candidates"))[:3]]
        guard_rows = [row for day in block_dates for row in _ready_rows(by_date[day].get("guard_candidates"))[:3]]
...
def run_holdout(
    ...
    *,
    ...
    ready_evaluable_dates = sum(1 for item in date_results if item.get("status") == "OK" and int((item.get("frozen_rule_meta") or {}).get("ready_count") or 0) > 0)
    full_top3_ready_dates = sum(1 for item in date_results if item.get("status") == "OK" and int((item.get("frozen_rule_meta") or {}).get("ready_count") or 0) >= 3)
    return {
        ...
        "holdout": {
            ...
            "ready_evaluable_date_count": ready_evaluable_dates,
            "full_top3_ready_date_count": full_top3_ready_dates,
...
def _markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- READY-evaluable dates: `{holdout.get('ready_evaluable_date_count')}` (full READY Top3: `{holdout.get('full_top3_ready_date_count')}`)",
        ...
        f"- Event-R dates WIN / TIE / LOSS / NO_READY: `{outcomes.get('WIN',0)} / {outcomes.get('TIE',0)} / {outcomes.get('LOSS',0)} / {outcomes.get('NO_READY',0)}`",
...
def write_outputs(payload: dict[str, Any], output_dir: Path) -> dict[str, str]:
    ...
    preferred_fields = [
        ...
        "candidate_state", "action", "strategy", "risk_status", "risk_warning",
        "conditions_missing", "atr14_pct", "return_std_20d_pct", "max_drawdown_20d_pct",
        "close_location", "ma20_gap_change_5d_pct", "volume_ratio_prev20",
        "volume_ratio_prev20_band", "volume_q25", "volume_low", "guard_rule",
        "return_5d", "mfe_5d", "mae_5d", "event_5d", "event_r_5d",
        "return_10d", "mfe_10d", "mae_10d", "event_10d", "event_r_10d",
        "return_20d", "mfe_20d", "mae_20d", "event_20d", "event_r_20d",
    ]
    present = {key for row in csv_rows for key in row.keys()}
    fieldnames = [name for name in preferred_fields if name in present]
    fieldnames.extend(sorted(present - set(fieldnames)))
    if not fieldnames:
        fieldnames = ["analysis_date", "code", "name"]
