...
class FakeStore:
    ...
    def seed(self, market: str, compact: str, *, stock: bool = True, index: bool = True) -> None:
        if stock:
            ...
            self.checked.add((market, compact, "stock"))
        if index:
            self.index_days[(market, compact)] = {"date": compact, "name": market}
            self.checked.add((market, compact, "index"))

    def mark_empty(self, market: str, compact: str, kind: str) -> None:
        self.checked.add((market, compact, kind))
    ...
    def day_complete(self, market: str, bas_dd: str, kind: str) -> bool:
        return (market, bas_dd, kind) in self.checked

    def put_stock_day(self, market: str, bas_dd: str, rows, *, stable: bool) -> int:
        normalized = list(rows)
        if normalized:
            self.stock_days[(market, bas_dd)] = normalized
        else:
            self.stock_days.pop((market, bas_dd), None)
        if stable:
            self.checked.add((market, bas_dd, "stock"))
    ...
    def put_index_day(self, market: str, bas_dd: str, row, *, stable: bool) -> None:
        if row is not None:
            ...
            self.index_days.pop((market, bas_dd), None)
        if stable:
            self.checked.add((market, bas_dd, "index"))
...
@pytest.mark.asyncio
async def test_empty_markers_do_not_qualify_as_analysis_ready_date() -> None:
    """Reproduce the real 9/15 false-success bug.

    A previously checked-but-empty 9/15 must not make freshness claim that 9/15
    is ready. When the provider now has real 9/15 data, prepare must re-fetch and
    replace the empty markers with actual data before returning UPDATED.
    """
    store = FakeStore()
    for market in ("KOSPI", "KOSDAQ"):
        store.seed(market, "20260914")
        store.mark_empty(market, "20260915", "stock")
        store.mark_empty(market, "20260915", "index")

    krx = FakeKrx(date(2026, 9, 16), date(2026, 9, 15))
    service = service_with(store, krx)

    assert service._date_complete_for_markets(["KOSPI", "KOSDAQ"], "2026-09-15") is False

    result = await service.prepare_latest_confirmed_data(
        market_scope="ALL", known_data_date="2026-09-14"
    )
    ...
    assert result["date_changed"] is True
    for market in ("KOSPI", "KOSDAQ"):
        assert store.latest_complete_date(market, "stock", "20260915") == "20260915"
        assert store.latest_complete_date(market, "index", "20260915") == "20260915"


@pytest.mark.asyncio
async def test_empty_markers_never_produce_false_new_date_notice() -> None:
    """If 9/15 is still empty at the provider, keep the real 9/14 data date."""
    store = FakeStore()
    for market in ("KOSPI", "KOSDAQ"):
        store.seed(market, "20260914")
        store.mark_empty(market, "20260915", "stock")
    ...
    krx = FakeKrx(date(2026, 9, 16), date(2026, 9, 14))
    result = await service_with(store, krx).prepare_latest_confirmed_data(
        market_scope="ALL", known_data_date="2026-09-14"
    )
