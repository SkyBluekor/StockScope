...
def main() -> int:
    ...
    try:
        ...
        smoke = (
            ...
            "db = Path('backend/runtime/market_history/market_history.db')\n"
            ...
            "          AND s.stock_code='005930'\n"
            "          AND EXISTS (\n"
            "              SELECT 1 FROM day_status ds\n"
            "              WHERE ds.market=s.market AND ds.bas_dd=s.bas_dd\n"
            "                AND ds.kind='stock' AND ds.status='data'\n"
            "          )\n"
            "          AND EXISTS (\n"
            "              SELECT 1 FROM day_status di\n"
            "              WHERE di.market=s.market AND di.bas_dd=s.bas_dd\n"
            "                AND di.kind='index' AND di.status='data'\n"
            "          )\n"
            "    \"\"\").fetchone()\n"
            "finally:\n"
            "    conn.close()\n"
            "bas_dd = row[0] if row else None\n"
            "assert bas_dd, 'No common confirmed KOSPI/005930 EOD date in Market Store'\n"
            "market_date = f'{bas_dd[:4]}-{bas_dd[4:6]}-{bas_dd[6:8]}'\n"
            "print('SMOKE_DATE_SELECTED:', market_date)\n"
            "r = analyze_single_stock(market='KOSPI', ticker='005930', market_date=market_date)\n"
