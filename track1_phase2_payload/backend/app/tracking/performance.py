...
def calculate_performance(
    ...
    *,
    ...
    """Calculate post-recommendation performance using D+1 and later only.

    `observations` represent market trading days. A day may have no stock bar
    (suspension/missing data), but it still counts toward the 5D/10D/20D horizon.
    The recommendation day itself is ignored defensively even if supplied.
    """
    ordered = sorted(
    ...
    for index, obs in enumerate(ordered, start=1):
        ...
        if not entry_touched and recommendation.entry_price is not None:
            if low <= recommendation.entry_price <= high:
                entry_touched = True
