...
function errorText(error: unknown) {
  ...
  const known: Record<string, string> = {
    ...
    SIM_SESSION_NOT_FOUND: "먼저 검증 기간을 설정해주세요.",
...
export default function SimulationWorkspace() {
  ...
  function openBuyDialog(mode: "recommend" | "search" = "recommend") {
    if (!session) {
      setMessage("먼저 검증 기간을 설정해주세요. 거래일을 정하면 종목 가격을 자동으로 불러옵니다.");
      return;
    }
    resetBuyDialog();
    setBuyMode(mode);
    ...
    <section className="simulation-workspace">
      <header className="sim-page-head">
        <div>
          <span className="sim-eyebrow">과거 시뮬레이션</span>
          <h1>과거 전략 검증</h1>
          <p>{portfolio.name} · 과거 확정 일봉으로 전략의 매수·매도 결과를 검증합니다.</p>
        </div>
      ...
      {!session && (
        <section className="sim-session-setup">
          <div><span className="sim-section-kicker">기간 설정</span><h2>검증 기간 설정</h2><p>확정 시장 데이터가 존재하는 거래일을 입력합니다.</p></div>
      ...
      <section className="sim-positions-section">
        ...
        {positions.length === 0 ? (
          <div className="sim-empty"><strong>현재 보유 중인 종목이 없습니다.</strong><span>{session ? "추천 종목을 고르거나 직접 찾아서 추가할 수 있습니다." : "먼저 검증 기간을 설정해주세요."}</span></div>
        ) : (
      ...
      <section className="sim-timeline">
        <div><span className="sim-section-kicker">날짜 진행</span><h2>{session ? dateText(session.current_date) : "검증 기간을 먼저 설정하세요"}</h2><p>다음 거래일의 확정 종가로 열린 보유 종목을 다시 평가합니다.</p></div>
